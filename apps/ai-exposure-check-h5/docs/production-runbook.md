...
- 新版 `sk-ws-` Key 使用百炼 API Key 页面显示的业务空间专属 OpenAI compatible 地址。生产 Key 的自定义模型范围只勾选 `qwen3.7-plus` 与 `deepseek-v4-pro`，并保留服务器 IP 白名单。
- `DEEPSEEK_ENABLED`、`QWEN_ENABLED`、`HY3_ENABLED`、`DOUBAO_ENABLED` 可独立停用。后台确认免费额度耗尽、过期或可能进入后付费时，先关闭对应开关再重启服务。
- 2026-07-11 百炼控制台确认 `deepseek-v4-pro` 剩余 `1,000,000 / 1,000,000`、`qwen3.7-plus` 剩余 `977,829 / 1,000,000`，均在 `2026-10-10` 到期并已开启“免费额度用完即停”；额度用完会返回 `403 AllocationQuota.FreeTierOnly`，不会进入百炼后付费。
...
- 现有每 IP 每小时 1 份、全局每天 30 份的产品限流继续作为免费额度保护上限，但不能替代云平台账单与余额检查。
...
- 首页返回 200。
- `/api/health` 返回 `model=deepseek-v4-pro`、`configuredProviderCount=4`、`sampleConcurrency=5`、`sampleMaxRetries=1`、`polishEnabled=false`，provider 列表只包含 DeepSeek、混元、千问、豆包且均为 `ready`。

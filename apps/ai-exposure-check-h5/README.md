...
新版 `sk-ws-` 百炼 Key 应使用 API Key 页面显示的业务空间专属 OpenAI compatible 地址，并在自定义权限中同时勾选 `qwen3.7-plus` 和 `deepseek-v4-pro`。生产 Key 继续使用服务器 IP 白名单，不要为本地调试放宽为全网可访问。
...
2026-07-11 百炼控制台确认：`deepseek-v4-pro` 免费额度剩余 `1,000,000 / 1,000,000`、`qwen3.7-plus` 剩余 `977,829 / 1,000,000`，两者均在 `2026-10-10` 到期且已开启“免费额度用完即停”。额度是时点数据，后台状态才是权威值。
